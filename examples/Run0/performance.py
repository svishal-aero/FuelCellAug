...
for i in range(beg, end):
  ...
  currDist = np.loadtxt("currDist_%04d.txt" % int(i))
  ...
  call('rm -f currDist_%04d.txt' % int(i), shell=True)

  error_baseline_water = np.linalg.norm(baseline-data)
  error_augmented_water = np.linalg.norm(augmented-data)
  #metric = error_baseline / (error_baseline + error_augmented)

  error_baseline_current = np.linalg.norm(baseline_current-data_current)
  error_augmented_current = np.linalg.norm(augmented_current-data_current)
  #metric2 = error_baseline / (error_baseline + error_augmented)

  diff_augmented_water = np.linalg.norm(augmented-baseline)
  diff_augmented_current = np.linalg.norm(augmented_current-baseline_current)

  with open('performance_logs/metrics.%d' % rank, 'a') as f:
    f.write('%4d %.10le %.10le %.10le %.10le %.10le %.10le\n' % \
    (i, error_baseline_water, error_augmented_water, error_baseline_current, error_augmented_current, diff_augmented_water, diff_augmented_current))
